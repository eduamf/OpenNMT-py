...
class ServerModel(object):
    ...
    def __init__(self, opt, model_id, tokenizer_opt=None, load=False,
                 timeout=-1, on_timeout="to_cpu", model_root="./", model_name="translator"):
    ...
    def run(self, inputs):
        ...
        if len(texts_to_translate) > 0:
            try:
                for line_to_translate in texts_to_translate:
                    line_to_translate = [line_to_translate]
                    # Translation
                    line_score, line_pred = self.translator.translate(
                        line_to_translate,
                        batch_size=self.opt.batch_size)
                    predictions.append(line_pred[0][0])
                    scores.append([line_score[0][0]])
                predictions = [["\n".join(predictions)]]
        ...
        # The lines are been concatenated preserving carriage return
